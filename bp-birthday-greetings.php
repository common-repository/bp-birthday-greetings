...
function bp_birthday_greetings_page_callback_section() {
	?>
	<p class="description"><?php echo esc_html__( 'Select DOB Field for which greetings will be sent.', 'bp-birthday-greetings' ); ?></p>
	<?php
}

/**
 * Option for date field.
 *
 * @since 1.0.0
 */
		...
		if ( bp_has_profile() ) :
			while ( bp_profile_groups() ) : bp_the_profile_group();
				while ( bp_profile_fields() ) : bp_the_profile_field();
					?>
					<option value="<?php bp_the_profile_field_id(); ?>" <?php if ( $bp_birthday_option_value == bp_get_the_profile_field_id() ): ?> selected <?php endif; ?>><?php bp_the_profile_field_name(); ?></option>
					<?php
				endwhile;
			endwhile;
		endif;
...
function bp_birthday_dpw_field_callback() {
	...
	?>
	<input type="number" name="bp-dp-width" value="<?php echo esc_html( $bp_birthday_dpw_value ); ?>" min="0">
	<?php
}

/**
 * Profile pic height field.
 *
 * @since 1.0.0
 */
function bp_birthday_dph_field_callback() {
	...
	?>
	<input type="number" name="bp-dp-height" value="<?php echo esc_html( $bp_birthday_dph_value ); ?>" min="0">
	<?php
}

/**
 * Profile pic type field.
 *
 * @since 1.0.0
 */
function bp_birthday_dpt_field_callback() {
	...
	<select name="bp-dp-type">
		<option value="thumb" <?php if ( 'thumb' === $bp_birthday_dpt_value ) : ?> selected <?php endif; ?>>Thumbnail</option>
		<option value="full" <?php if ( 'full' === $bp_birthday_dpt_value ) : ?> selected <?php endif; ?>>Full Width</option>
	</select>
	<p class="note"> <?php __( 'For more information please check our FAQ', 'bp-birthday-greetings' ); ?> <a target="_blank" href="https://wordpress.org/plugins/bp-birthday-greetings/#faq"><?php __( 'here', 'bp-birthday-greetings' ); ?></a>.</p>
	<?php
}